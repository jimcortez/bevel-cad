...
from bevel_cad.examples.finger_sensor_holder import (
    TUNED_DEFAULTS,
...
def test_params_from_config_overlay(tmp_path) -> None:
    overlay = tmp_path / "part.yaml"
    overlay.write_text("finger_sensor_holder:\n  holder_width: 31.5\n")
    cfg = load_layers(files=[overlay], part_defaults={"finger_sensor_holder": TUNED_DEFAULTS}, project_config=None, user_config=False).cfg
    params = FingerSensorHolderParams.from_config(cfg)
    assert params.holder_width == 31.5
    assert params.sensor_pocket_diameter == 16.30  # tuned default
    assert params.hinge_axis_height_above_base == 10.7
    assert FingerSensorHolderParams.from_config({}).sensor_pocket_diameter == 17.20


def test_build_returns_named_assembly() -> None:
    cfg = load_layers(part_defaults={"finger_sensor_holder": TUNED_DEFAULTS}, project_config=None, user_config=False).cfg
    assy = build(cfg)
    assert [n for n, node in assy.traverse() if node.obj is not None][:2] == ["holder", "retention_plate"]

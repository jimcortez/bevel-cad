...
``pyrender`` (and therefore OpenGL/EGL) is imported lazily inside the render
functions so that importing this module never touches a GPU context.
Camera / light / colour parameters come from any object exposing the
:class:`bevel_cad.config.schema.PreviewSettings` attributes.
"""
...
def _pyrender():
    """Import pyrender on first use, after selecting the EGL platform when headless."""
    _configure_headless_opengl()
    import pyrender  # noqa: PLC0415 - deliberate lazy import
...
def render_glb_to_image(
    ...
    r, g, b = preview_config.color_rgb
    scale = 0.82
    color_rgb = (float(r) * scale, float(g) * scale, float(b) * scale)
    opacity = max(0.0, min(1.0, float(preview_config.opacity)))
    ...
    bg_r, bg_g, bg_b = preview_config.background_rgb
    ...
    # Camera
    width = max(1, int(preview_config.image_width))
    height = max(1, int(preview_config.image_height))
    aspect = width / height
    camera = pyrender.PerspectiveCamera(yfov=math.pi / 4.0, aspectRatio=aspect)
    cam_pose = _camera_pose_from_view(
        center,
        distance,
        elevation_deg=float(preview_config.elevation),
        azimuth_deg=float(preview_config.azimuth),
        roll_deg=float(preview_config.roll),
        world_up_axis="y",
    )
    scene.add(camera, pose=cam_pose)

    # Light (directional from front/side); same Y-up as camera
    light_az = math.radians(float(preview_config.light_azimuth))
    light_el = math.radians(float(preview_config.light_elevation))
    d = 2.0 * distance

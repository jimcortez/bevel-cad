"""
Finger pulse-sensor holder: a hinged trough that cradles a fingertip over a round
PCB sensor, plus a retention plate. Every dimension lives in the
``finger_sensor_holder`` config block (millimetres); the dataclass below holds the
defaults and :func:`parameters_for_prototype_tuning` lists what usually needs a
tweak after the first test print.
"""
...
from cadquery.func import *  # noqa: F403 - functional API style

import bevel_cad
from bevel_cad.mesh.validate import require_watertight, validate_solids_watertight  # noqa: F401 - re-exported
...
class FingerSensorHolderParams:
    ...
    @classmethod
    def from_config(cls, cfg: Any) -> "FingerSensorHolderParams":
        """Read the ``finger_sensor_holder`` block of a config (mapping or DictConfig)."""
        data = cfg.get("finger_sensor_holder") if hasattr(cfg, "get") else getattr(cfg, "finger_sensor_holder", None)
        if data is None:
            return cls()
        kwargs = {}
...
# Values tuned on printed prototypes (differ from the dataclass defaults above).
TUNED_DEFAULTS: Dict[str, Any] = {
    **asdict(FingerSensorHolderParams()),
    "hinge_axis_height_above_base": 10.7,
    "sensor_pocket_diameter": 16.30,
}
...
@bevel_cad.part(
    defaults={"finger_sensor_holder": TUNED_DEFAULTS},
    description="Hinged finger pulse-sensor holder + retention plate (validated watertight)",
)
def build(cfg: Any) -> cq.Assembly:
    parts = build_finger_sensor_holder_parts(config=cfg)
    require_watertight(
        [("holder", parts.holder), ("retention_plate", parts.retention_plate)], what="Finger sensor holder"
    )
    logger.info(
        "Finger sensor holder validation OK (watertight). Tune after first print: %s",
        ", ".join(parameters_for_prototype_tuning()[:6]) + ", ...",
    )
    return parts.to_assembly()

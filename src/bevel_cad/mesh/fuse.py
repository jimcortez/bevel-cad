...
Builders that assemble many small solids (braids, studs, lattices) return a
``Compound`` for speed. Before export or mesh analysis those solids are
boolean-fused into one body so downstream stages see a single watertight-ish
part rather than hundreds of disconnected lumps.
"""
